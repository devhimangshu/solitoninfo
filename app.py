...
async def send_all_likes(target_uid, server_name, url):
    ...
    tasks = []
    for acc in fresh_accounts[:50]:  # Limit to 50 fresh accounts per request
...
def handle_requests():
    ...
    try:
        ...
        return jsonify({
            ...
            "fresh_used": result.get('fresh_used', 0),
...
def reset_cache():
    ...
    global liked_cache
    liked_cache.clear()
    return jsonify({"message": "Cache cleared", "credit": "@juli_dvrma"})
...
if __name__ == '__main__':
    ...
    print("   - account_bd.txt (BD server)")
